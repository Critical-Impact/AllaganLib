...
public abstract class IconButtonColumn<TConfiguration, TData, TMessageBase> : IntegerFormField<TConfiguration>, IValueColumn<TConfiguration, TData, TMessageBase, int?>
    where TConfiguration : IConfigurable<int?>, INotifyPropertyChanged
{
    private readonly ITextureProvider textureProvider;

    protected IconButtonColumn(ITextureProvider textureProvider, ImGuiService imGuiService)
        : base(imGuiService)
    {
        ...
        if (ImGui.TableGetColumnFlags().HasFlag(ImGuiTableColumnFlags.IsEnabled))
        {
            ...
            if (currentValue != null)
            {
                ...
                if (ImGui.ImageButton(
                        this.textureProvider.GetFromGameIcon(new GameIconLookup((uint)currentValue, isHq))
    ...
    public abstract void OnButtonClick(TData item);
}
